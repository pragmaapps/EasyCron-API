...
function add(task, expression, API_URL) {
	return new Promise((resolve, reject) => {
		...
		if (minute === "*" && hour === "*" && day === "*" && month === "*") {
			reject(new Error("Provide atleast minute, hour, day or month in input"));
		}
		let queryString;
		if (expression) {
...
function easyCron(config = {}) {
	...
	return {
		...
		enable: (task) => state (task, API_URL + "enable?token=" + token),
		disable: (task) => state (task, API_URL + "disable?token=" + token),
		delete: (task) => state (task, API_URL + "delete?token=" + token)
	}
